...
class TryOnModel:
    ...
    def _upper_body_landmarks(self, frame_rgb: np.ndarray):
        """Single MediaPipe Pose call → return upper-body landmark dict
        in PIXEL coordinates, or None if any are missing / low visibility.

        Returns dict with keys:
            l_shoulder, r_shoulder, l_elbow, r_elbow, l_wrist, r_wrist
        Each value is np.array([x, y], dtype=float32).

        Single call so we don't re-run pose for shoulder_angle + sleeve
        warp + shoulder seam separately.
        """
        ...
        try:
            ...
            H, W = frame_rgb.shape[:2]
            ids = dict(l_shoulder=11, r_shoulder=12,
                       l_elbow=13,    r_elbow=14,
                       l_wrist=15,    r_wrist=16)
            out = {}
            for name, idx in ids.items():
                p = lmk[idx]
                # We use shoulders strictly; elbows/wrists allowed to be
                # lower-visibility because hands often go behind the body.
                vis_floor = 0.5 if "shoulder" in name else 0.2
                if p.visibility < vis_floor:
                    return None
                out[name] = np.array([p.x * W, p.y * H], dtype=np.float32)
            return out
        except Exception:
            return None

    @staticmethod
    def _warp_sleeve_to_arm(garment_sleeve_rgba: np.ndarray,
                            shoulder_xy: np.ndarray,
                            elbow_xy: np.ndarray,
                            wrist_xy: np.ndarray,
                            sleeve_thickness: float,
                            canvas_size: tuple) -> tuple:
        """Piecewise-warp a sleeve image (with alpha) so its centre-line
        runs shoulder → elbow → wrist on the target canvas.

        Approach: two affine warps stacked.
            Upper half of sleeve image  → quad (shoulder ± thickness) → (elbow ± thickness)
            Lower half of sleeve image  → quad (elbow ± thickness)    → (wrist ± thickness)
        Each quad is built by walking perpendicular to the bone vector by
        `sleeve_thickness`. The two halves overlap by ~6 px at the elbow
        seam (smoothed by the alpha feather inside the sleeve image), so
        the bend reads as a continuous fabric crease.

        Args:
            garment_sleeve_rgba: (H, W, 4) — sleeve image with alpha channel
            shoulder_xy, elbow_xy, wrist_xy: target points on canvas
            sleeve_thickness: half-width in px of the warped sleeve
            canvas_size: (H, W) of the destination canvas
        Returns:
            (rgb_canvas, alpha_canvas) — both float32 the shape of canvas_size
        """
        canvas_h, canvas_w = canvas_size
        sh, sw = garment_sleeve_rgba.shape[:2]
        rgb_canvas   = np.zeros((canvas_h, canvas_w, 3), np.float32)
        alpha_canvas = np.zeros((canvas_h, canvas_w),    np.float32)

        # Source quads (top half of sleeve image, bottom half)
        mid_sy = sh // 2
        src_upper = np.float32([[0, 0], [sw, 0], [sw, mid_sy], [0, mid_sy]])
        src_lower = np.float32([[0, mid_sy], [sw, mid_sy], [sw, sh], [0, sh]])

        # Helper: build a destination quad from a bone (a→b) using
        # perpendicular `thickness` on both sides.
        def _bone_quad(a, b, thickness):
            v = b - a
            n = np.linalg.norm(v) + 1e-6
            # Perpendicular unit vector (rotate v by 90°)
            perp = np.array([-v[1], v[0]], dtype=np.float32) / n
            a_l = a + perp * thickness
            a_r = a - perp * thickness
            b_l = b + perp * thickness
            b_r = b - perp * thickness
            # Order: top-left, top-right, bottom-right, bottom-left
            return np.float32([a_l, a_r, b_r, b_l])

        dst_upper = _bone_quad(shoulder_xy, elbow_xy, sleeve_thickness)
        dst_lower = _bone_quad(elbow_xy,    wrist_xy, sleeve_thickness)

        for src_quad, dst_quad in [(src_upper, dst_upper), (src_lower, dst_lower)]:
            M = cv2.getPerspectiveTransform(src_quad, dst_quad)
            warped = cv2.warpPerspective(
                garment_sleeve_rgba, M, (canvas_w, canvas_h),
                flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT,
                borderValue=(0, 0, 0, 0),
            )
            seg_rgb   = warped[:, :, :3].astype(np.float32)
            seg_alpha = warped[:, :, 3].astype(np.float32) / 255.0
            # Layered composite — later segment over earlier
            a = seg_alpha[:, :, None]
            rgb_canvas = seg_rgb * a + rgb_canvas * (1.0 - a)
            alpha_canvas = seg_alpha + alpha_canvas * (1.0 - seg_alpha)

        return rgb_canvas, np.clip(alpha_canvas, 0.0, 1.0)
    ...
    def _infer_live_geometric(self, person_image: Image.Image) -> Image.Image:
        ...
        # ── Per-arm sleeve warping ───────────────────────────────────────────
        # This is the "mirror" upgrade: when the user moves their arm, the
        # sleeve follows shoulder → elbow → wrist rather than staying as a
        # static rectangle. Skipped silently if MediaPipe Pose is missing
        # or hand visibility is too low.
        landmarks = self._upper_body_landmarks(frame)
        if landmarks is not None:
            try:
                # Split the garment image into L / R sleeves (left 25%,
                # right 25% in the original garment). Each sleeve image
                # carries an alpha channel so background doesn't leak.
                g_arr = np.array(garment.convert("RGB"))
                if self._garment_alpha is not None:
                    a_full = (self._garment_alpha * 255).astype(np.uint8)
                else:
                    gg = cv2.cvtColor(g_arr, cv2.COLOR_RGB2GRAY)
                    _, bg = cv2.threshold(gg, 250, 255, cv2.THRESH_BINARY)
                    a_full = 255 - bg
                a_full = cv2.GaussianBlur(a_full, (9, 9), 0)

                gH_, gW_ = g_arr.shape[:2]
                sleeve_w = int(gW_ * 0.25)
                left_sleeve_rgba  = np.dstack([g_arr[:, :sleeve_w],            a_full[:, :sleeve_w]])
                right_sleeve_rgba = np.dstack([g_arr[:, gW_ - sleeve_w:],       a_full[:, gW_ - sleeve_w:]])

                # Sleeve thickness = function of shoulder-to-shoulder span,
                # so it scales with how close the user is to the camera.
                shoulder_span = float(np.linalg.norm(
                    landmarks["r_shoulder"] - landmarks["l_shoulder"]
                ))
                sleeve_thickness = max(8.0, shoulder_span * 0.22)

                canvas_size = (H, W)
                # Right sleeve (subject's anatomical right = viewer's left
                # when camera is selfie-flipped, but mediapipe gives us
                # consistent x coords regardless — keep names aligned with
                # garment image halves).
                right_rgb, right_alpha = self._warp_sleeve_to_arm(
                    right_sleeve_rgba,
                ...
                left_rgb, left_alpha = self._warp_sleeve_to_arm(
                    left_sleeve_rgba,
                ...
                # Subtract the hand mask from the sleeve alpha so the
                # actual hand stays visible at the end of each sleeve.
                if hand_full is not None:
                    left_alpha  = left_alpha  * (1.0 - hand_full)
                    right_alpha = right_alpha * (1.0 - hand_full)

                # Composite sleeves on top of the torso paste
                a_l = left_alpha[:, :, None]
                a_r = right_alpha[:, :, None]
                result_f = result.astype(np.float32)
                result_f = left_rgb  * a_l + result_f * (1.0 - a_l)
                result_f = right_rgb * a_r + result_f * (1.0 - a_r)
                result = result_f.astype(np.uint8)
            except Exception as e:
                log.debug(f"sleeve warp skipped: {e}")
